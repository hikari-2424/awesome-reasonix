...
Respond with:
1. Position 鈥?1-2 sentences
2. Reasoning 鈥?3 concise bullets
3. Risk 鈥?biggest risk in your recommendation
4. Surprise 鈥?one thing the other voices may miss
...
- `santa-method` 鈥?adversarial verification
- `knowledge-ops` 鈥?persist durable decision deltas correctly
- `search-first` 鈥?gather external reference material before the council if needed
- `architecture-decision-records` 鈥?formalize the outcome when the decision becomes long-lived system policy
