...
```bash
# Environment setup 鈥?source before use
...
- **Store OAuth secrets securely** 鈥?not in source code or logs.
...
- `brand-voice` 鈥?Build a reusable voice profile from real X and site/source material
- `content-engine` 鈥?Generate platform-native content for X
- `crosspost` 鈥?Distribute content across X, LinkedIn, and other platforms
- `connections-optimizer` 鈥?Reorganize the X graph before drafting network-driven outreach
