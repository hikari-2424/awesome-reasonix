...
description: iOS 26 Liquid Glass design system 鈥?dynamic glass material with blur, reflection, and interactive morphing for SwiftUI, UIKit, and WidgetKit.
---

# Liquid Glass Design System (iOS 26)

Patterns for implementing Apple's Liquid Glass 鈥?a dynamic material that blurs content behind it, reflects color and light from surrounding content, and reacts to touch and pointer interactions. Covers SwiftUI, UIKit, and WidgetKit integration.
...
- `.regular` 鈥?standard glass effect
- `.tint(Color)` 鈥?add color tint for prominence
- `.interactive()` 鈥?react to touch and pointer interactions
...
The `spacing` parameter controls merge distance 鈥?closer elements blend their glass shapes together.
...
To allow horizontal scroll content to extend under a sidebar or inspector, ensure the `ScrollView` content reaches the leading/trailing edges of the container. The system automatically handles the under-sidebar scrolling behavior when the layout extends to the edges 鈥?no additional modifier is needed.

## Core Pattern 鈥?UIKit
...
| `spacing` parameter | Controls merge distance 鈥?fine-tune how close elements must be to blend |
| `@Namespace` + `glassEffectID` | Enables smooth morphing transitions on view hierarchy changes |
| `interactive()` modifier | Explicit opt-in for touch/pointer reactions 鈥?not all glass should respond |
...
- **Always use GlassEffectContainer** when applying glass to multiple sibling views 鈥?it enables morphing and improves rendering performance
...
- **Test across appearances** 鈥?light mode, dark mode, and accented/tinted modes
- **Ensure accessibility contrast** 鈥?text on glass must remain readable

## Anti-Patterns to Avoid

- Using multiple standalone `.glassEffect()` views without a GlassEffectContainer
- Nesting too many glass effects 鈥?degrades performance and visual clarity
- Applying glass to every view 鈥?reserve for interactive elements, toolbars, and cards
- Forgetting `clipsToBounds = true` in UIKit when using corner radii
- Ignoring accented rendering mode in widgets 鈥?breaks tinted Home Screen appearance
- Using opaque backgrounds behind glass 鈥?defeats the translucency effect
