...
1. **Null safety** 鈥?avoid `!`, prefer `?.`/`??`/pattern matching
2. **Immutable state** 鈥?sealed classes, `freezed`, `copyWith`
3. **Async composition** 鈥?concurrent `Future.wait`, safe `BuildContext` after `await`
4. **Widget architecture** 鈥?extract to classes (not methods), `const` propagation, scoped rebuilds
5. **State management** 鈥?BLoC/Cubit events, Riverpod notifiers and derived providers
6. **Navigation** 鈥?GoRouter with reactive auth guards via `refreshListenable`
7. **Networking** 鈥?Dio with interceptors, token refresh with one-time retry guard
8. **Error handling** 鈥?global capture, `ErrorWidget.builder`, crashlytics wiring
9. **Testing** 鈥?unit (BLoC test), widget (ProviderScope overrides), fakes over mocks

## Examples

```dart
// Sealed state 鈥?prevents impossible states
...
```dart
// BAD 鈥?crashes at runtime if null
final name = user!.name;

// GOOD 鈥?provide fallback
final name = user?.name ?? 'Unknown';

// GOOD 鈥?Dart 3 pattern matching (preferred for complex cases)
final display = switch (user) {
  User(:final name, :final email) => '$name <$email>',
  null => 'Guest',
};

// GOOD 鈥?guard early return
...
```dart
// BAD 鈥?defers null error to runtime
late String userId;

// GOOD 鈥?nullable with explicit initialization
String? userId;

// OK 鈥?use late only when initialization is guaranteed before first access
...
// Exhaustive switch 鈥?compiler enforces all branches
...
Future<DashboardData> loadDashboard(UserRepository users, OrderRepository orders) async {
  // Run concurrently 鈥?don't await sequentially
...
// In widget layer 鈥?declarative, no manual subscription
...
```dart
// CRITICAL 鈥?always check mounted after any await in StatefulWidget
Future<void> _handleSubmit() async {
  setState(() => _isLoading = true);
  try {
    await authService.login(_email, _password);
    if (!mounted) return; // 鈫?guard before using context
    context.go('/home');
...
```dart
// BAD 鈥?private method returning widget, prevents optimization
Widget _buildHeader() {
  return Container(
...
// GOOD 鈥?separate widget class, enables const, element reuse
...
```dart
// BAD 鈥?entire page rebuilds on every counter change
...
// GOOD 鈥?isolate the rebuilding part
...
```dart
// Cubit 鈥?synchronous or simple async state
...
```dart
// Global error capture 鈥?set up in main()
void main() {
...
- Skill: `flutter-dart-code-review` 鈥?comprehensive review checklist
- Rules: `rules/dart/` 鈥?coding style, patterns, security, testing, hooks
