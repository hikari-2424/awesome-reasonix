...
description: Build a fully automated AI-powered data collection agent for any public source 鈥?job boards, prices, news, GitHub, sports, anything. Scrapes on a schedule, enriches data with a free LLM (Gemini Flash), stores results in Notion/Sheets/Supabase, and learns from user feedback. Runs 100% free on GitHub Actions. Use when the user wants to monitor, collect, or track any public data automatically.
---
...
**Stack: Python 路 Gemini Flash (free) 路 GitHub Actions (free) 路 Notion / Sheets / Supabase**
...
```
COLLECT 鈫?ENRICH 鈫?STORE
  鈹?          鈹?       鈹?Scraper    AI (LLM)  Database
...
| **AI enrichment** | Gemini Flash via REST API | 500 req/day, 1M tokens/day 鈥?free |
...
```
gemini-2.0-flash-lite (30 RPM) 鈫?gemini-2.0-flash (15 RPM) 鈫?gemini-2.5-flash (10 RPM) 鈫?gemini-flash-lite-latest (fallback)
```
...
for item in items:
    result = call_ai(item)  # 33 calls 鈫?hits rate limit

# GOOD: 7 API calls for 33 items (batch size 5)
for batch in chunks(items, size=5):
    results = call_ai(batch)  # 7 calls 鈫?stays within free tier
```
...
- Job boards 鈫?score relevance to resume
- Product prices 鈫?alert on drops
- GitHub repos 鈫?summarise new releases
- News feeds 鈫?classify by topic + sentiment
- Sports results 鈫?extract stats to tracker
- Events calendar 鈫?filter by interest
...
```
my-agent/
鈹溾攢鈹€ config.yaml              # User customises this (keywords, filters, preferences)
鈹溾攢鈹€ profile/
鈹?  鈹斺攢鈹€ context.md           # User context the AI uses (resume, interests, criteria)
鈹溾攢鈹€ scraper/
鈹?  鈹溾攢鈹€ __init__.py
鈹?  鈹溾攢鈹€ main.py              # Orchestrator: scrape 鈫?enrich 鈫?store
鈹?  鈹溾攢鈹€ filters.py           # Rule-based pre-filter (fast, before AI)
鈹?  鈹斺攢鈹€ sources/
鈹?      鈹溾攢鈹€ __init__.py
鈹?      鈹斺攢鈹€ source_name.py   # One file per data source
鈹溾攢鈹€ ai/
鈹?  鈹溾攢鈹€ __init__.py
鈹?  鈹溾攢鈹€ client.py            # Gemini REST client with model fallback
鈹?  鈹溾攢鈹€ pipeline.py          # Batch AI analysis
鈹?  鈹溾攢鈹€ jd_fetcher.py        # Fetch full content from URLs (optional)
鈹?  鈹斺攢鈹€ memory.py            # Learn from user feedback
鈹溾攢鈹€ storage/
鈹?  鈹溾攢鈹€ __init__.py
鈹?  鈹斺攢鈹€ notion_sync.py       # Or sheets_sync.py / supabase_sync.py
鈹溾攢鈹€ data/
鈹?  鈹斺攢鈹€ feedback.json        # User decision history (auto-updated)
鈹溾攢鈹€ .env.example
鈹溾攢鈹€ setup.py                 # One-time DB/schema creation
鈹溾攢鈹€ enrich_existing.py       # Backfill AI scores on old rows
鈹溾攢鈹€ requirements.txt
鈹斺攢鈹€ .github/
    鈹斺攢鈹€ workflows/
        鈹斺攢鈹€ scraper.yml      # GitHub Actions schedule
```
...
"""
[Source Name] 鈥?scrapes [what] from [where].
...
def analyse_batch(items: list[dict], context: str = "", preference_prompt: str = "") -> list[dict]:
    ...
    print(f"  [AI] {len(items)} items 鈫?{len(batches)} API calls")
...
def get_existing_urls(db_id: str) -> set[str]:
    """Fetch all URLs already stored 鈥?used for deduplication."""
...
def main():
    ...
    if ai_enabled() and deduped:
        ...
        print("[AI] Skipped 鈥?GEMINI_API_KEY not set")

    added, skipped = sync(db_id, deduped)
    print(f"Done 鈥?{added} new, {skipped} existing")
...
on:
  schedule:
    - cron: "0 */3 * * *"  # every 3 hours 鈥?adjust to your needs
...
```yaml
# Customise this file 鈥?no code changes needed
...
# Your priorities 鈥?AI uses these for scoring
priorities:
  - "example priority 1"
  - "example priority 2"
...
- [ ] `config.yaml` controls all user-facing settings 鈥?no hardcoded values
...
- [ ] Batch size 鈮?5 items per API call
- [ ] `maxOutputTokens` 鈮?2048
- [ ] `.env` is in `.gitignore`
...
"Track new GitHub repos tagged with 'llm' or 'agents' 鈥?summarise each one"
"Collect Chief of Staff job listings from LinkedIn and Cutshort into Notion"
"Monitor a subreddit for posts mentioning my company 鈥?classify sentiment"
...
"Build a real estate listing watcher 鈥?alert on new properties under 鈧? Cr"
```
...
100% free on GitHub Actions. Follow Steps 1鈥? above to build your own.
