...
|------|---------|---------|
| Quick Scan | `results.json` exists (default) | 5鈥?0 min |
| Full Stocktake | `results.json` absent, or `/skill-stocktake full` | 20鈥?0 min |

**Results cache:** `~/.claude/skills/skill-stocktake/results.json`

## Quick Scan Flow

Re-evaluate only skills that have changed since the last run (5鈥?0 min).
...
```
Scanning:
  鉁?~/.claude/skills/         (17 files)
  鉁?{cwd}/.claude/skills/    (not found 鈥?global skills only)
```

| Skill | 7d use | 30d use | Description |
|-------|--------|---------|-------------|

### Phase 2 鈥?Quality Evaluation
...
Evaluation is **holistic AI judgment** 鈥?not a numeric rubric. Guiding dimensions:
- **Actionability**: code examples, commands, or steps that let you act immediately
- **Scope fit**: name, trigger, and content are aligned; not too broad or narrow
- **Uniqueness**: value not replaceable by MEMORY.md / CLAUDE.md / another skill
- **Currency**: technical references work in the current environment

**Reason quality requirements** 鈥?the `reason` field must be self-contained and decision-enabling:
- Do NOT write "unchanged" alone 鈥?always restate the core evidence
...
- For **Improve**: describe the specific change needed (what section, what action, target size if relevant)
  - Bad: `"Too long"`
  - Good: `"276 lines; Section 'Framework Comparison' (L80鈥?40) duplicates ai-era-architecture-principles; delete it to reach ~150 lines."`
- For **Keep** (mtime-only change in Quick Scan): restate the original verdict rationale, do not write "unchanged"
  - Bad: `"Unchanged"`
  - Good: `"mtime updated but content unchanged. Unique Python reference explicitly imported by rules/python/; no overlap found."`

### Phase 3 鈥?Summary Table
...
2. **Improve**: present specific improvement suggestions with rationale:
   - What to change and why (e.g., "trim 430鈫?00 lines because sections X/Y duplicate python-patterns")
