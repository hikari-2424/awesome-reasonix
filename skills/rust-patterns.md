...
fn process_bad(data: &Vec<u8>) -> usize {
    let cloned = data.clone(); // Wasteful 鈥?just borrow
    cloned.len()
}
```
...
### Use `Result` and `?` 鈥?Never `unwrap()` in Production
...
### Exhaustive Matching 鈥?No Catch-All for Business Logic
...
// Collect Results 鈥?short-circuits on first error
...
鈹?  鈹溾攢鈹€ main.rs
鈹?  鈹溾攢鈹€ lib.rs
鈹?  鈹溾攢鈹€ auth/          # Domain module
鈹?  鈹?  鈹溾攢鈹€ mod.rs
鈹?  鈹?  鈹溾攢鈹€ token.rs
鈹?  鈹?  鈹斺攢鈹€ middleware.rs
鈹?  鈹溾攢鈹€ orders/        # Domain module
鈹?  鈹?  鈹溾攢鈹€ mod.rs
鈹?  鈹?  鈹溾攢鈹€ model.rs
鈹?  鈹?  鈹斺攢鈹€ service.rs
鈹?  鈹斺攢鈹€ db/            # Infrastructure
鈹?      鈹溾攢鈹€ mod.rs
鈹?      鈹斺攢鈹€ pool.rs
鈹溾攢鈹€ tests/             # Integration tests
鈹溾攢鈹€ benches/           # Benchmarks
鈹斺攢鈹€ Cargo.toml
```

### Visibility 鈥?Expose Minimally
...
**Remember**: If it compiles, it's probably correct 鈥?but only if you avoid `unwrap()`, minimize `unsafe`, and let the type system work for you.
