...
- Transitioning a ticket status (To Do 鈫?In Progress 鈫?Done)
...
**Add to your MCP config** (e.g., `~/.claude.json` 鈫?`mcpServers`):
...
```json
{
  "jira": {
    ...
    "description": "Jira issue tracking 鈥?search, create, update, comment, transition"
  }
}
```
...
3. Copy the token 鈥?store it in your environment, never in source code
...
> **Tip:** Always call `jira_get_transitions` before transitioning 鈥?transition IDs vary per project workflow.
...
- **Functional requirements** 鈥?What the feature does
- **Acceptance criteria** 鈥?Conditions that must be met
- **Testable behaviors** 鈥?Specific actions and expected outcomes
- **User roles** 鈥?Who uses this feature and their permissions
- **Data requirements** 鈥?What data is needed
- **Integration points** 鈥?APIs, services, or systems involved

### 2. Test Types Needed
- **Unit tests** 鈥?Individual functions and utilities
- **Integration tests** 鈥?API endpoints and service interactions
- **E2E tests** 鈥?User-facing UI flows
- **API tests** 鈥?Endpoint contracts and error handling
...
Unit Tests:
- [test file 1] 鈥?[what it covers]
- [test file 2] 鈥?[what it covers]

Integration Tests:
- [test file] 鈥?[endpoints/flows covered]
...
- **Validate** that credentials are set before making API calls 鈥?fail fast with a clear message
...
- Link rather than copy 鈥?point to PRs, test reports, and dashboards
