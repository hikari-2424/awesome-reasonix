...
1. **Identify target code** 鈥?Find the function, class, or module to test
2. **Write a Kotest spec** 鈥?Choose a spec style (StringSpec, FunSpec, BehaviorSpec) matching the test scope
3. **Mock dependencies** 鈥?Use MockK to isolate the unit under test
4. **Run tests (RED)** 鈥?Verify the test fails with the expected error
5. **Implement code (GREEN)** 鈥?Write minimal code to pass the test
6. **Refactor** 鈥?Improve the implementation while keeping tests green
7. **Check coverage** 鈥?Run `./gradlew koverHtmlReport` and verify 80%+ coverage
...
- **Kotest specs** 鈥?StringSpec, FunSpec, BehaviorSpec, DescribeSpec examples in [Kotest Spec Styles](#kotest-spec-styles)
- **Mocking** 鈥?MockK setup, coroutine mocking, argument capture in [MockK](#mockk)
- **TDD walkthrough** 鈥?Full RED/GREEN/REFACTOR cycle with EmailValidator in [TDD Workflow for Kotlin](#tdd-workflow-for-kotlin)
- **Coverage** 鈥?Kover configuration and commands in [Kover Coverage](#kover-coverage)
- **Ktor testing** 鈥?testApplication setup in [Ktor testApplication Testing](#ktor-testapplication-testing)
