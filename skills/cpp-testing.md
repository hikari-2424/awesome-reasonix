...
- **TDD loop**: red 鈫?green 鈫?refactor (tests first, minimal fix, then cleanups).
...
Follow the RED 鈫?GREEN 鈫?REFACTOR loop:
...
- **Using fixed temp paths** 鈫?Generate unique temp directories per test and clean them.
- **Relying on wall clock time** 鈫?Inject a clock or use fake time sources.
- **Flaky concurrency tests** 鈫?Use condition variables/latches and bounded waits.
- **Hidden global state** 鈫?Reset global state in fixtures or remove globals.
- **Over-mocking** 鈫?Prefer fakes for stateful behavior and only mock interactions.
- **Missing sanitizer runs** 鈫?Add ASan/UBSan/TSan builds in CI.
- **Coverage on debug-only builds** 鈫?Ensure coverage targets use consistent flags.
