...
- `位` is the decay factor, usually `0.5`
...
```text
B_ext(m) = B(m) + 伪 路 危_{m' 鈭?N(m) \\ M} 危_{t 鈭?T} w(t) 路 位^(d(m',t))
```

Where:

- `N(m) \\ M` is the set of people the mutual knows that you do not
- `伪` discounts second-order reach, usually `0.3`
...
- `尾` is the engagement bonus, usually `0.2`
