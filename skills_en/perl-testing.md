...
# Step 2: GREEN 鈥?Write minimal implementation
# lib/Calculator.pm
package Calculator;
use v5.36;
use Moo;
...
# Step 3: REFACTOR 鈥?Improve while tests stay green
# Run: prove -lv t/unit/calculator.t
```

## Test::More Fundamentals

The standard Perl testing module 鈥?widely used, ships with core.
...
Test2::V0 is the modern replacement for Test::More 鈥?richer assertions, better diagnostics, and extensible.
...
# Hash builder 鈥?check partial structure
is(
    $user->to_hash,
    hash {
        field name  => 'Alice';
...
is(
    ...
    array {
        ...
        item DNE();  # Does Not Exist 鈥?verify no extra items
    },
    'result matches expected list'
);

# Bag 鈥?order-independent comparison
is(
    $tags,
    bag {
        item 'perl';
...
```text
t/
鈹溾攢鈹€ 00-load.t              # Verify modules compile
鈹溾攢鈹€ 01-basic.t             # Core functionality
鈹溾攢鈹€ unit/
鈹?  鈹溾攢鈹€ config.t           # Unit tests by module
鈹?  鈹溾攢鈹€ user.t
鈹?  鈹斺攢鈹€ util.t
鈹溾攢鈹€ integration/
鈹?  鈹溾攢鈹€ database.t
鈹?  鈹斺攢鈹€ api.t
鈹溾攢鈹€ lib/
鈹?  鈹斺攢鈹€ TestHelper.pm      # Shared test utilities
鈹斺攢鈹€ fixtures/
    鈹溾攢鈹€ config.json        # Test data files
    鈹斺攢鈹€ users.csv
```
...
# *MyApp::API::fetch_user = sub { ... };  # NEVER 鈥?leaks across tests
```
...
use Test2::V0;
is(1, 1, 'works');
# Missing done_testing 鈥?silent bugs if test code is skipped
...
Use `my` variables inside subtests 鈥?never `our` 鈥?to prevent state leaking between tests.
