...
description: Swift 6.2 Approachable Concurrency 鈥?single-threaded by default, @concurrent for explicit background offloading, isolated conformances for main actor types.
---
...
```swift
// Swift 6.2: OK 鈥?async stays on MainActor, no data race
@MainActor
final class StickerModel {
    let photoProcessor = PhotoProcessor()
...
## Core Pattern 鈥?Isolated Conformances
...
// Swift 6.1: ERROR 鈥?crosses into main actor-isolated code
...
```swift
// OK 鈥?ImageExporter is also @MainActor
@MainActor
...
// ERROR 鈥?nonisolated context can't use MainActor conformance
...
## Core Pattern 鈥?Global and Static Variables

Protect global/static state with MainActor:

```swift
// Swift 6.1: ERROR 鈥?non-Sendable type may have shared mutable state
...
Swift 6.2 introduces a mode where MainActor is inferred by default 鈥?no manual annotations needed:
...
## Core Pattern 鈥?@concurrent for Background Work

When you need actual parallelism, explicitly offload with `@concurrent`:

> **Important:** This example requires Approachable Concurrency build settings 鈥?SE-0466 (MainActor default isolation) and SE-0461 (NonisolatedNonsendingByDefault). With these enabled, `extractSticker` stays on the caller's actor, making mutable state access safe. **Without these settings, this code has a data race** 鈥?the compiler will flag it.
...
| Opt-in adoption | Non-breaking migration path 鈥?enable features incrementally |
...
- **Start on MainActor** 鈥?write single-threaded code first, optimize later
- **Use `@concurrent` only for CPU-intensive work** 鈥?image processing, compression, complex computation
- **Enable MainActor inference mode** for app targets that are mostly single-threaded
- **Profile before offloading** 鈥?use Instruments to find actual bottlenecks
- **Protect globals with MainActor** 鈥?global/static mutable state needs actor isolation
- **Use isolated conformances** instead of `nonisolated` workarounds or `@Sendable` wrappers
- **Migrate incrementally** 鈥?enable features one at a time in build settings
...
- Fighting the compiler 鈥?if it reports a data race, the code has a real concurrency issue
...
- Performance optimization 鈥?offloading specific heavy computations to background
