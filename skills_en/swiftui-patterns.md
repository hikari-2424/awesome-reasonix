...
Use `@Observable` (not `ObservableObject`) 鈥?it tracks property-level changes so SwiftUI only re-renders views that read the changed property:
...
Always use stable, unique IDs in `ForEach` 鈥?avoid using array indices:
...
- Use `.task {}` for async work 鈥?it cancels automatically when the view disappears
- Use `.sensoryFeedback()` and `.geometryGroup()` sparingly in scroll views
- Minimize `.shadow()`, `.blur()`, and `.mask()` in lists 鈥?they trigger offscreen rendering
...
- Using `ObservableObject` / `@Published` / `@StateObject` / `@EnvironmentObject` in new code 鈥?migrate to `@Observable`
- Putting async work directly in `body` or `init` 鈥?use `.task {}` or explicit load methods
- Creating view models as `@State` inside child views that don't own the data 鈥?pass from parent instead
- Using `AnyView` type erasure 鈥?prefer `@ViewBuilder` or `Group` for conditional views
