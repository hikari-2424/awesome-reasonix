...
description: Compose Multiplatform and Jetpack Compose patterns for KMP projects 鈥?state management, navigation, theming, performance, and platform-specific UI.
---
...
private fun ItemListContent(
    ...
    // Stateless composable 鈥?easy to preview and test
}
```
...
// In Composable 鈥?single lambda instead of many
ItemListContent(
    state = state,
...
Modifier order matters 鈥?apply in this sequence:
...
```kotlin
// BAD 鈥?new lambda and list every recomposition
items.filter { it.isActive }.forEach { ActiveItem(it, onClick = { handle(it) }) }

// GOOD 鈥?key each item so callbacks stay attached to the right row
...
- Passing `NavController` deep into composables 鈥?pass lambda callbacks instead
- Heavy computation inside `@Composable` functions 鈥?move to ViewModel or `remember {}`
- Using `LaunchedEffect(Unit)` as a substitute for ViewModel init 鈥?it re-runs on configuration change in some setups
- Creating new object instances in composable parameters 鈥?causes unnecessary recomposition
