...
Testing patterns specifically designed for AI-assisted development, where the same model writes code and reviews it 鈥?creating systematic blind spots that only automated tests can catch.

## When to Activate

- AI agent (Claude Code, Cursor, Codex) has modified API routes or backend logic
- A bug was found and fixed 鈥?need to prevent re-introduction
...
```
AI writes fix 鈫?AI reviews fix 鈫?AI says "looks correct" 鈫?Bug still exists
```
...
Fix 1: Added notification_settings to API response
  鈫?Forgot to add it to the SELECT query
  鈫?AI reviewed and missed it (same blind spot)

Fix 2: Added it to SELECT query
  鈫?TypeScript build error (column not in generated types)
  鈫?AI reviewed Fix 1 but didn't catch the SELECT issue

Fix 3: Changed to SELECT *
  鈫?Fixed production path, forgot sandbox path
  鈫?AI reviewed and missed it AGAIN (4th occurrence)
...
```typescript
// __tests__/setup.ts
// Force sandbox mode 鈥?no database needed
...
// Define the contract 鈥?what fields MUST be in the response
const REQUIRED_FIELDS = [
  ...
  "created_at",
  "avatar_url",
  "notification_settings",  // 鈫?Added after bug found it missing
];
...
describe("GET /api/user/profile", () => {
  ...
  // Regression test 鈥?this exact bug was introduced by AI 4 times
...
- If tests fail 鈫?report as highest priority bug
- If build fails 鈫?report type errors as highest priority
...
```
User: "銉愩偘銉併偋銉冦偗銇椼仸" (or "/bug-check")
  鈹?  鈹溾攢 Step 1: npm run test
  鈹?  鈹溾攢 FAIL 鈫?Bug found mechanically (no AI judgment needed)
  鈹?  鈹斺攢 PASS 鈫?Continue
  鈹?  鈹溾攢 Step 2: npm run build
  鈹?  鈹溾攢 FAIL 鈫?Type error found mechanically
  鈹?  鈹斺攢 PASS 鈫?Continue
  鈹?  鈹溾攢 Step 3: AI code review (with known blind spots in mind)
  鈹?  鈹斺攢 Findings reported
  鈹?  鈹斺攢 Step 4: For each fix, write a regression test
      鈹斺攢 Next bug-check catches if fix breaks
```
...
// 鈫?notification_settings is always undefined
...
**Frequency**: Moderate 鈥?when adding error handling to existing components
...
```
Bug found in /api/user/profile     鈫?Write test for profile API
Bug found in /api/user/messages    鈫?Write test for messages API
Bug found in /api/user/favorites   鈫?Write test for favorites API
No bug in /api/user/notifications  鈫?Don't write test (yet)
```
...
4. Test count grows organically with bug fixes 鈥?no wasted effort
...
- Aim for coverage percentage 鈥?aim for regression prevention
