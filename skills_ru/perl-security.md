...
sub bad_untaint($input) {
    $input =~ /^(.*)$/s;
    return $1;  # Accepts ANYTHING 鈥?pointless
}
```
...
# Good: Allowlist 鈥?define exactly what's permitted
...
# Bad: Blocklist 鈥?always incomplete
...
# Good: List form 鈥?no shell interpolation
sub run_command(@cmd) {
    system(@cmd) == 0
...
# Bad: String form 鈥?shell injection!
...
# Good: Parameterized queries 鈥?always use placeholders
...
# <%= $user_input %>   鈥?auto-escaped (safe)
# <%== $raw_html %>    鈥?raw output (dangerous, use only for trusted content)

# Template auto-escaping (Template Toolkit)
# [% user_input | html %]  鈥?explicit HTML encoding
...
Use constant-time comparison when verifying tokens. Most web frameworks (Mojolicious, Dancer2, Catalyst) provide built-in CSRF protection 鈥?prefer those over hand-rolled solutions.
...
```ini
# .perlcriticrc 鈥?security-focused configuration
severity = 3
theme = security + core
...
($input) = $input =~ /(.*)/s;           # Lazy untaint 鈥?defeats purpose
...
**Remember**: Perl's flexibility is powerful but requires discipline. Use taint mode for web-facing code, validate all input with allowlists, use DBI placeholders for every query, and encode all output for its context. Defense in depth 鈥?never rely on a single layer.
