...
When writing inline Python (`python -c "..."`), always use properly formatted code 鈥?use semicolons to separate statements and keep it readable. For anything longer than ~3 statements, use a heredoc instead:
...
# index_scenes() has no force parameter 鈥?it raises an error if a scene
...
| `generate_video()` / `create_collection()` fails | `Operation not allowed` or `maximum limit` | Plan-gated features 鈥?inform the user about plan limits |
...
| Need to change resolution (e.g. 1080p 鈫?720p) | `conn.transcode()` with `VideoConfig(resolution=720)` |
