...
description: Apple FoundationModels framework for on-device LLM 鈥?text generation, guided generation with @Generable, tool calling, and snapshot streaming in iOS 26+.
---

# FoundationModels: On-Device LLM (iOS 26)

Patterns for integrating Apple's on-device language model into apps using the FoundationModels framework. Covers text generation, structured output with `@Generable`, custom tool calling, and snapshot streaming 鈥?all running on-device for privacy and offline support.
...
## Core Pattern 鈥?Availability Check
...
## Core Pattern 鈥?Guided Generation with @Generable
...
- `.range(0...20)` 鈥?numeric range
- `.count(3)` 鈥?array element count
- `description:` 鈥?semantic guidance for generation

## Core Pattern 鈥?Tool Calling
...
## Core Pattern 鈥?Snapshot Streaming
...
| Decision | Rationale |
|----------|-----------|
| On-device execution | Privacy 鈥?no data leaves the device; works offline |
...
| `response.content` (not `.output`) | Correct API 鈥?always access results via `.content` property |

## Best Practices

- **Always check `model.availability`** before creating a session 鈥?handle all unavailability cases
- **Use `instructions`** to guide model behavior 鈥?they take priority over prompts
- **Check `isResponding`** before sending a new request 鈥?sessions handle one request at a time
- **Access `response.content`** for results 鈥?not `.output`
- **Break large inputs into chunks** 鈥?4,096 token limit applies to instructions + prompt + output combined
- **Use `@Generable`** for structured output 鈥?stronger guarantees than parsing raw strings
- **Use `GenerationOptions(temperature:)`** to tune creativity (higher = more creative)
- **Monitor with Instruments** 鈥?use Xcode Instruments to profile request performance
...
- Building complex multi-step logic in a single prompt 鈥?break into multiple focused prompts
- Assuming the model is always available 鈥?device eligibility and settings vary
