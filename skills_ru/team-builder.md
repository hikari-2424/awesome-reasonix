...
**Subdirectory layout** 鈥?domain is inferred from the folder name:

```
agents/
鈹溾攢鈹€ engineering/
鈹?  鈹溾攢鈹€ security-engineer.md
鈹?  鈹斺攢鈹€ software-architect.md
鈹溾攢鈹€ marketing/
鈹?  鈹斺攢鈹€ seo-specialist.md
鈹斺攢鈹€ sales/
    鈹斺攢鈹€ discovery-coach.md
```

**Flat layout** 鈥?domain inferred from shared filename prefixes. A prefix counts as a domain when 2+ files share it. Files with unique prefixes go to "General". Note: the algorithm splits at the first `-`, so multi-word domains (e.g., `product-management`) should use the subdirectory layout instead:

```
agents/
鈹溾攢鈹€ engineering-security-engineer.md
鈹溾攢鈹€ engineering-software-architect.md
鈹溾攢鈹€ marketing-seo-specialist.md
鈹溾攢鈹€ marketing-content-strategist.md
鈹溾攢鈹€ sales-discovery-coach.md
鈹斺攢鈹€ sales-outbound-strategist.md
```
...
1. **`claude agents` command** (primary) 鈥?run `claude agents` to get all agents known to the CLI, including user agents, plugin agents (e.g. `everything-claude-code:architect`), and built-in agents. This automatically covers ECC marketplace installs without any path configuration.
2. **File glob** (fallback, for reading agent content) 鈥?agent markdown files are read from:
   - `./agents/**/*.md` + `./agents/*.md` 鈥?project-local agents
   - `~/.claude/agents/**/*.md` + `~/.claude/agents/*.md` 鈥?global user agents
...
1. Engineering 鈥?Software Architect, Security Engineer
2. Marketing 鈥?SEO Specialist
3. Sales 鈥?Discovery Coach, Outbound Strategist
...
3. Spawn all agents in parallel using the Agent tool:
   ...
   - Each agent runs independently 鈥?no inter-agent communication needed
4. If an agent fails (error, timeout, or empty output), note the failure inline (e.g., "Security Engineer: failed 鈥?[reason]") and continue with results from agents that succeeded
...
- **Parallel dispatch.** All agents run simultaneously 鈥?use the Agent tool's parallel invocation pattern.
...
Claude:
Available agent domains:
1. Engineering (2) 鈥?Software Architect, Security Engineer
2. Marketing (1) 鈥?SEO Specialist
3. Sales (4) 鈥?Discovery Coach, Outbound Strategist, Proposal Strategist, Sales Engineer
4. Support (1) 鈥?Executive Summary
