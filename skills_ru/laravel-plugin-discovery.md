...
No API key required 鈥?the server is free for the Laravel community.
...
- `health_score` (string, optional): Filter by health band 鈥?`Healthy`, `Medium`, `Unhealthy`, or `Unrated`
- `laravel_compatibility` (string, optional): Filter by Laravel version 鈥?`"5"`, `"6"`, `"7"`, `"8"`, `"9"`, `"10"`, `"11"`, `"12"`, `"13"`
- `php_compatibility` (string, optional): Filter by PHP version 鈥?`"7.4"`, `"8.0"`, `"8.1"`, `"8.2"`, `"8.3"`, `"8.4"`, `"8.5"`
...
1. **Always filter by health** 鈥?Use `health_score: "Healthy"` for production projects
2. **Match Laravel version** 鈥?Always check `laravel_compatibility` matches the target project
3. **Check vendor reputation** 鈥?Prefer packages from known vendors (spatie, laravel, etc.)
4. **Review before recommending** 鈥?Use GetPluginDetailsTool for a comprehensive assessment
5. **No API key needed** 鈥?The MCP is free, no authentication required

---

## Related Skills

- `laravel-patterns` 鈥?Laravel architecture and patterns
- `laravel-tdd` 鈥?Test-driven development for Laravel
- `laravel-security` 鈥?Laravel security best practices
- `documentation-lookup` 鈥?General library documentation lookup (Context7)
